...
func youtube_country_data_text(c *gin.Context) {
	...
	encoding_keyword := url.QueryEscape(req.Country)
	url := fmt.Sprintf("https://www.googleapis.com/youtube/v3/videos?part=snippet,statistics&chart=mostPopular&maxResults=48&regionCode=%s&key=%s", encoding_keyword, YOUTUBE_API_KEY)
	resp, err := http.Get(url)
	if err != nil {
		fmt.Println("Error making HTTP request:", err)
	}
...
func main() {
	...
	router.Use(cors.New(cors.Config{
		AllowOrigins:     []string{"https://youtube-trends-finder.vercel.app/"}, // 許可するサイト
		AllowMethods:     []string{"GET", "POST", "OPTIONS"},                    // 許可するHTTPメソッド
	...
	router.POST("/youtube/country_trend/data", youtube_country_data_text)
